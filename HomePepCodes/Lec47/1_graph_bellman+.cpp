...
bool is_comp_bp(vve& g, int src, vector<int>& visited){
    // visited holds the level now, not true or false
    list<BP> q;
...
int timer = 0;
void bridges_APs(vve& g, int src, vector<bool>& visited, vector<int>& discovery,
                vector<int>& low, vector<bool>& aps, int par){
    ...
    for(int i = 0; i < g[src].size(); i++){
        ...
        if(visited[nbr] == false){
            counter++;
            // cout << "E Pre " << src;
            bridges_APs(g, nbr, visited, discovery, low, aps, src);
            // cout << "E Post " << src;
            ...
            if(discovery[src] == 1){
                ...
                if(low[nbr] > discovery[src]){
                    cout << src << " to " << nbr << " is a bridge." << endl;
                }
            }
...
bool is_cycle = false;
void topo_slave(vve& g, vector<bool>& visited, stack<int>& st, int src, int parent){
    visited[src] = true;
    for(int i = 0; i < g[src].size(); i++){
        if(visited[g[src][i].n] && parent != g[src][i].n){
            is_cycle = true;
            return;
        }
        
        if(!visited[g[src][i].n])
            topo_slave(g, visited, st, g[src][i].n, src);
    }
...
void topo_master(vve& g){
    vector<bool> visited (g.size(), false);
    stack<int> st;
    while(st.size() != g.size()){
        int r = (rand() % g.size()) + 0;
        if(visited[r] == false){
            topo_slave(g, visited, st, r, -1);
        }
        if(is_cycle){
            cout << "Cycle Present: No Topologically Sorted Sequence Exits" << endl;
            return;
        }
    }
...
int main(int argc, char** argv){
    ...
    // bridges_APs(graph, 2, visited, discovery, low, aps, -1);
    ...
    // fwg[3].push_back(Edge (0,10));

    // floyd_warshal(fwg);
    // bellman_ford(fwg, 0);

    // vve tg (2);
    // tg[0].push_back(Edge (1,3));
    // tg[1].push_back(Edge (0,13));
