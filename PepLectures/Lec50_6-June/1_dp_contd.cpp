...
int knapsack01(int cap, vector<int>& wts, vector<int>& prices){
    vector<vector<int>> strg (wts.size() + 1, vector<int> (cap+1, 0));

    for(int i = 1; i < strg.size(); i++){
        for(int j = 1; j < strg[0].size(); j++){
            if(wts[i-1] <= j){
                int myMax = max(strg[i-1][j], strg[i-1][j-wts[i-1]] + prices[i-1]);
                strg[i][j] = myMax;
            } else {
                strg[i][j] = strg[i-1][j];
            }
...
int unbounded_KS(int cap, vector<int>& wts, vector<int>& prices){
    vector<int> strg (cap+1, 0);

    for(int i = 1; i <= cap; i++){
        for(int j = 0; j < wts.size(); j++){
            if(i >= wts[j]){
                strg[i] = max(strg[i - wts[j]] + prices[j], strg[i]);
            }
        }
    }
...
int rod_cutting(vector<int>& profit){
    vector<int> strg (profit.size(), 0);
    ...
    for(int i = 2; i < strg.size(); i++){
        ...
        while(left <= right){
            if(strg[left] + strg[right] > myMax){
                myMax = profit[left] + strg[right];
            }
            // cout << myMax << endl;
            left++;
            right--;
...
int main(int argc, char** argv){
    ...
    // knapsack01(7, wts, prices);
    // unbounded_KS(7, wts, prices);

    // vector<int> profit {
    //     0,3,5,6,15,10,25,12,24
    // };

    // cout << rod_cutting(profit) << endl;
}