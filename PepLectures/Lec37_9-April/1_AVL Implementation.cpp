...
// This add will not make a balanced tree when called iteratively. This is the problem with this add. Will be handled in AVL
...
Node* remove(Node* root, int data){
    ...
    if(data > root->data){
        ...
        // 2 children case
            ...
            root->left = remove(root->left, max_left);
        }
    }
...
// The following code has very bad complexity. DP will solve this. Maybe.k
int catalan_number_count_of_BST(int n){
    if(n == 0 || n == 1){
        return 1;
    }
    ...
    int count = 0;
    for(int i = 0; i < n; i++){
        count += catalan_number_count_of_BST(i) * catalan_number_count_of_BST(n - i - 1);
    }
...
int main(int argc, char** argv)
{
    ...
    cout << catalan_number_count_of_BST(4) << endl;
}